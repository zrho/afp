...
scoreGrid = do
  CleverAI {rules = r, tracking = t} <- get
  return $ {- trace' showScoreGrid $ -} scoreGrid' r t

scoreGrid' :: Rules -> TrackingGrid -> ScoreGrid
scoreGrid' r t = array
...
sunkFleet :: TrackingGrid -> [Pos] -> FleetPlacement
...
shipMatchesTracking :: Int            -- ^ safety margin from the rules
                    -> TrackingGrid  -- ^ AI's tracking field
                    -> FleetPlacement -- ^ already sunk fleet
                    -> ShipShape      -- ^ ship to be tested
                    -> Bool
